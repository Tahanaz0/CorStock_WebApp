...
import StockImage from "../../assets/images/switch-vertical-02.png";

const StockLevelFilter = () => {
  const stockLevelList = ["All", "In Stock", "Out of Stock", "Low Stock"];
  const [selectedStockLevels, setSelectedStockLevels] = useState<string[]>([]);

  const handleToggle = (value: string) => {
    setSelectedStockLevels((current) =>
        ...
        <Accordion.Item value="tags">
          ...
          <Accordion.Panel className="pb-3">
            <Stack gap={6} className="mt-0 ml-1">
              {stockLevelList.map((name) => (
                <Checkbox
                  key={name}
                  label={name}
                  checked={selectedStockLevels.includes(name)}
