...
import priceImage from "../../assets/images/priceValue.png";

const PriceValueFilter = () => {
  const priceList = ["All", "$0 - $50", "$50 - $100", "$100+"];
            ...
            <Stack gap={6} className="mt-0 ml-1">
              {priceList.map((name) => (
                <Checkbox
                  key={name}
                  label={name}
                  checked={selectedPrices.includes(name)}
...
export default PriceValueFilter;
