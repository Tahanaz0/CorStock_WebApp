...
interface UsersTabProps {
  ...
  onActionMenuClick?: (rowData: UserData, event: React.MouseEvent<HTMLButtonElement>) => void;
}
    ...
    <>
      ...
      <div className="flex gap-4 mb-6 flex-wrap">
        {stats.map((item, idx) => (
          <div
            key={idx}
            className="border border-[#EAECF0] rounded-lg p-3 bg-white w-44 h-22"
          >
            ...
            <tbody className="divide-y divide-gray-100">
              {data.map((row, idx) => (
                <tr key={idx} className="text-sm hover:bg-gray-50">
                  ...
                  <td className="px-3 py-4 text-gray-600">{row.role}</td>
                  <td className="px-3 py-4 text-gray-600">{row.email}</td>
                  <td className="px-3 py-4 text-gray-600">{row.siteAccess}</td>
                  <td className="px-3 py-4 text-gray-600 whitespace-nowrap">
                    {row.lastLogin}
                  </td>
                  <td className="px-3 py-4">{renderStatusBadge(row.status)}</td>
